# Client Test Accounts — Google SSO (credo-group.co.uk)

Account assignment for demo/testing between Fedor and Greg.

Shared password for all `credo-group.co.uk` accounts: `CdqXDZSKDVAr2qG`

| # | Email | Name | Role Title | Department | Group | Assigned To | Status |
|--:|-------|------|-----------|------------|-------|-------------|--------|
| 12 | `mark.director@credo-group.co.uk` | Mark Director | Director | Leadership | leadership | **Greg** | OK |
| 13 | `adam.finance@credo-group.co.uk` | Adam Finance | Finance Manager | Finance | finance | **Fedor** | OK |
| 14 | `kevin.operations@credo-group.co.uk` | Kevin Operations | Operations Manager | Operations | operations | **Greg** | OK |
| 15 | `damon.operations@credo-group.co.uk` | Damon Operations | Operations Supervisor | Operations | operations | **Fedor** | OK |
| 16 | `nicholas.field@credo-group.co.uk` | Nicholas Field | Field Engineer | Field Engineering | engineering | **Greg** | OK |
| 17 | `mathew.field@credo-group.co.uk` | Mathew Field | Field Engineer | Field Engineering | engineering | **Fedor** | OK |

### Split Summary

| Person | Accounts |
|--------|----------|
| **Greg** | Mark Director, Kevin Operations, Nicholas Field (3) |
| **Fedor** | Adam Finance, Damon Operations, Mathew Field (3) |

---

*Derived from [test-accounts.md](test-accounts.md).*
